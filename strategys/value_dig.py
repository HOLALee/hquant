...
def buy_stock(context,code):
    ...
    if len(context.portfolio.positions) < stock_buy_num:
        if stock_percentage > context.percentage:  #设置单支股票最大买入仓位
            ...
            order_percent(code, stock_percentage)     #买入股票
...
def select_stock(context):
    ...
    #1月~4月，取前一年3季度报
        ...
        Year = Year - 1
        q = 3
    logger.info(str(Year) + '-' + str(Month) + '-' + str(Day) + ':' + str(q))
    #查询最新股票数据,股票列表、业绩报告（主表）、盈利能力、成长能力
    data = ts.get_stock_basics()
    logger.info('get_stock_basics success')
